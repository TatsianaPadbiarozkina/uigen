...
function FileTreeNode({ node, level }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-neutral-700 cursor-pointer text-sm transition-colors",
          selectedFile === node.path && "bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400"
        )}
        ...
        {node.type === "directory" ? (
          <>
            {isExpanded ? (
              ...
              <ChevronRight className="h-3.5 w-3.5 shrink-0 text-gray-500 dark:text-neutral-400" />
            )}
            {isExpanded ? (
          ...
          <>
            <div className="w-3.5" />
            <FileCode className="h-4 w-4 shrink-0 text-gray-400 dark:text-neutral-500" />
          </>
        )}
        <span className="truncate text-gray-700 dark:text-neutral-300">{node.name}</span>
      </div>
  ...
  if (!rootNode || !rootNode.children || rootNode.children.size === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        <Folder className="h-12 w-12 text-gray-300 dark:text-neutral-600 mb-3" />
        <p className="text-sm text-gray-500 dark:text-neutral-400">No files yet</p>
        <p className="text-xs text-gray-400 dark:text-neutral-500 mt-1">Files will appear here</p>
      </div>
    );
  }
